...
mod tests {
    ...
    /// Helper: build a minimal test Packet for proof roundtrip tests.
    fn make_test_packet_for_receipt() -> crate::packet::Packet {
        use crate::hash::AddressHash;
        use crate::packet::{DestinationType, Header, PacketDataBuffer, PacketType};

        let mut data = PacketDataBuffer::new();
        data.safe_write(b"receipt test payload");
        ...
        crate::packet::Packet {
            ...
            destination: AddressHash::new([0xCD; 16]),
            data,
            ..Default::default()
        }
    }
    ...
    #[test]
    fn test_implicit_proof_roundtrip() {
        // Generate an implicit proof via PrivateIdentity::prove(),
        // then validate it against a PacketReceipt.
        let identity = PrivateIdentity::new_from_rand(OsRng);
        let packet = make_test_packet_for_receipt();
        let hash = packet.hash();

        let proof_packet = identity.prove(&packet, true);
        let proof_data = proof_packet.data.as_slice();

        // Should be implicit (signature only, 64 bytes)
    ...
    #[test]
    fn test_explicit_proof_roundtrip() {
        // Generate an explicit proof via PrivateIdentity::prove(),
        // then validate it against a PacketReceipt.
        let identity = PrivateIdentity::new_from_rand(OsRng);
        let packet = make_test_packet_for_receipt();
        let hash = packet.hash();

        let proof_packet = identity.prove(&packet, false);
        let proof_data = proof_packet.data.as_slice();

        // Should be explicit (hash + signature, 96 bytes)
