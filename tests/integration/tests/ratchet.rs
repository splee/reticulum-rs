...
/// Test that Python ratchet client can receive ratchet announces from Python server.
///
/// This verifies the end-to-end Python ratchet flow before testing Rust interop.
#[test]
fn test_python_ratchet_announce_roundtrip() {
    let mut ctx = IntegrationTestContext::new().expect("Failed to create test context");
    ...
    // Start Python ratchet client first (to listen)
    let client = ctx
        .run_python_helper(
            "python_ratchet_client.py",
            &[
                "--tcp-client",
                &format!("127.0.0.1:{}", hub.port()),
                "-a",
                ...
                "ratchettest",
                "-n",
                "1", // wait for 1 announce
                "-t",
                "40", // timeout
                "-v",
            ],
        )
        .expect("Failed to start Python ratchet client");

    // Wait for client to be listening
    let _ = client
        .wait_for_output("LISTENING=", Duration::from_secs(10))
        .expect("Client should start listening");
    ...
    // Wait for server to announce
    let _ = server
        .wait_for_output("ANNOUNCE_SENT=", Duration::from_secs(15))
        .expect("Server should announce");

    // Wait for client to receive announce
    let announce_received = client
        .wait_for_output("ANNOUNCE_RECEIVED=", Duration::from_secs(20))
        .expect("Client should receive announce");

    eprintln!("Client received: {}", announce_received);

    // Check for ratchet in received announce
    let client_output = client.output();
    eprintln!("Python ratchet client output:\n{}", client_output);

    // Verify the announce had a ratchet
    assert!(
        client_output.contains("ANNOUNCE_HAS_RATCHET=true"),
        "Received announce should have ratchet"
    );

    assert!(
        client_output.contains("ANNOUNCE_RATCHET_ID="),
        "Client should report ratchet ID"
    );

    // Extract and verify ratchet ID format
    if let Some(line) = client_output
        .lines()
        .find(|l| l.starts_with("ANNOUNCE_RATCHET_ID="))
    {
        let ratchet_id = line.trim_start_matches("ANNOUNCE_RATCHET_ID=");
        assert_eq!(
            ratchet_id.len(),
            20,
            "Ratchet ID should be 20 hex chars (10 bytes)"
        );
        eprintln!("Client received ratchet ID: {}", ratchet_id);
    }
}
